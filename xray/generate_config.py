...
if not VLESS_URL:
    print("ERROR: VLESS_URL is not set", file=sys.stderr)
    sys.exit(1)

url = VLESS_URL.split("#", 1)[0]
parsed = urlparse(url)
...
if parsed.scheme != "vless" or not uuid or not host or not port:
    print("ERROR: VLESS_URL is invalid", file=sys.stderr)
    sys.exit(1)
